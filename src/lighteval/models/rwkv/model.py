...
load(name="wind_backstepping", sources=[f'{basedir}/cuda/wkv7_cuda.cu', f'{basedir}/cuda/wkv7_op.cpp'],
...
class RWKV_Tmix_x070(nn.Module):
    ...
    def forward(self, x, v_first):
        ...
        x = x + ((r.view(B, T, H, -1) * k.view(B, T, H, -1) * self.r_k).sum(dim=-1, keepdim=True) * v.view(B, T, H,
                                                                                                           -1)).view(B,
                                                                                                                     T,
                                                                                                                     C)
        x = self.output(x * g)
...
class RWKV(pl.LightningModule):
    ...
    def generate(self, input_ids, do_sample, temperature, top_p, max_new_tokens, stop_token_idx=261) -> list[int]:
        ...
        with tqdm(total=max_new_tokens, postfix={'Throughput': 'N/A'}) as pbar:
            for i in range(max_new_tokens):
                ...
                input_ids = input_ids[:, -self.args.ctx_len:]  # truncate
                end_time = time.time()
                throughput = bsz / (end_time - start_time)
